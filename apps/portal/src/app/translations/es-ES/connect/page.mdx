...
Log in to the [thirdweb dashboard](https://thirdweb.com/team/~/~/projects). Click on Create New > Project to get your **Client ID**. You'll need your Client ID to interact with the Connect SDK.
...
If the destination token __can__ be bought directly with fiat, your users can onramp directly to their destination token.

If the destination token __can not__ be bought directly with fiat, your users will need to onramp to an intermediate token, then convert the onramp token to the destination token.
